  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const sourcesResponse = await fetch('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ messages: [...messages, userMessage, { ...assistantMessage, content: fullContent }], stream: false })
      });
      ...
      setMessages(prev => prev.map(msg => 
        msg.id === assistantMessage.id 
          ? { ...msg, content: fullContent, isStreaming: false, sources: sourcesData.sources }
          : msg
      ));
